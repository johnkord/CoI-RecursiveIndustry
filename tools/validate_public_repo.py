...
def validate_ui_icon_contract(errors: list[str], root: Path) -> None:
    ...
    if not isinstance(icons, list) or len(icons) != 91:
        errors.append(
            f"UI icon manifest must contain exactly 91 identities, found "
    ...
    if root_match is None:
        ...
        if len(constants) != 91:
            errors.append(
            f"C# source must declare exactly 91 UI icon constants, found {len(constants)}"
            )
    ...
    if review.get("runtime_status") != "OPEN_SUCCESSOR_0.23.0A":
        errors.append("UI icon runtime review must target the 0.23.0a successor")
...
def validate_source_contract(errors: list[str], root: Path) -> None:
    ...
    if catalog_path.is_file():
        ...
        if catalog.count("new UniversalFacilitySpec(") != 25:
            errors.append("generated catalog must contain 25 facilities")
        if catalog.count("new UniversalDirectBindingSpec(") != 231:
            errors.append("generated catalog must contain 231 Direct bindings")
